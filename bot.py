...
async def restart_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.warning("Admin issued /restart — restarting.")
...
async def kill_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logging.warning("Admin issued /kill — shutting down.")
    raise SystemExit(0)
...
async def run_bot():
    ...
    # Patch for partner/store custom-date text
...
def main_supervisor():
    while True:
        ...
        if exit_code == 0:
            ...
            break
        logging.warning(f"Bot crashed (exit {exit_code})— restarting in 5s.")
        time.sleep(5)
