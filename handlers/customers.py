...
async def edit_customer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return ConversationHandler.END
    buttons = [InlineKeyboardButton(f"{r['name']} ({r['currency']})", callback_data=f"edit_customer_{r.doc_id}") for r in rows]
...
async def get_edit_currency(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Save", callback_data="cust_conf_yes"), InlineKeyboardButton("❌ Cancel", callback_data="cust_conf_no")]
    ])
    await update.message.reply_text(
...
async def confirm_edit_customer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query.data == 'cust_conf_yes':
        ...
        secure_db.update('customers', {'name': context.user_data['new_name'], 'currency': context.user_data['new_cur']}, [rec.doc_id])
        await update.callback_query.edit_message_text(
...
async def delete_customer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return ConversationHandler.END
    buttons = [InlineKeyboardButton(f"{r['name']} ({r['currency']})", callback_data=f"delete_customer_{r.doc_id}") for r in rows]
    kb = InlineKeyboardMarkup([buttons[i:i+2] for i in range(0, len(buttons), 2)])
...
async def get_delete_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Yes, delete", callback_data="cust_del_yes"), InlineKeyboardButton("❌ No, cancel", callback_data="cust_del_no")]
    ])
    await update.callback_query.edit_message_text(
