...
(
    ...
    P_SEL_REMOVE, P_CONFIRM_REMOVE, P_SEL_VIEW
) = range(10)
...
def register_partner_handlers(app):
    partner_conv = ConversationHandler(
        ...
        states={
            ...
            P_CONFIRM:         [CallbackQueryHandler(confirm_partner)],
            P_SEL_EDIT:        [CallbackQueryHandler(select_edit_partner)],
            P_NEW_NAME:        [MessageHandler(filters.TEXT & ~filters.COMMAND, new_partner_name)],
            P_NEW_CUR:         [MessageHandler(filters.TEXT & ~filters.COMMAND, new_partner_currency)],
            P_CONFIRM_EDIT:    [CallbackQueryHandler(confirm_edit_partner)],
            P_SEL_REMOVE:      [CallbackQueryHandler(select_remove_partner)],
            P_CONFIRM_REMOVE:  [CallbackQueryHandler(confirm_remove_partner)],
            P_SEL_VIEW:        [CallbackQueryHandler(view_partner_details)],
        },
...
async def get_partner_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Partner name set to '{name}'. Now enter currency:")
    return P_CUR
...
async def select_edit_partner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return ConversationHandler.END

async def new_partner_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return ConversationHandler.END

async def new_partner_currency(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return ConversationHandler.END

async def confirm_edit_partner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return ConversationHandler.END

# --- Remove Partner (placeholder) ---
...
async def select_remove_partner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return ConversationHandler.END

async def confirm_remove_partner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return ConversationHandler.END

# --- View Partner (placeholder) ---
...
async def view_partner_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return ConversationHandler.END
