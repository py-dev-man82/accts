...
async def get_payout_fee(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data['fee_amt']  = data['local_amt'] * fee_pct / 100
...
async def get_payout_usd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("📝 Add Note", callback_data="note_yes"),
    ...
    if not rows:
        ...
        for r in rows:
            ...
            lines.append(f"[{r.doc_id}] {name}: {r['local_amt']:.2f} "
                         f"(fee {fee_pct:.2f}%={fee_amt:.2f}) → {usd:.2f} USD")
        text = "Payouts:\n" + "\n".join(lines)
...
async def get_edit_fee(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    d = context.user_data
    d['fee_perc'] = fee_pct
    d['fee_amt']  = d['local_amt'] * fee_pct / 100
    await update.message.reply_text(
...
async def get_edit_usd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['usd_amt'] = usd
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("📝 Add Note", callback_data="edit_note_yes"),
                                InlineKeyboardButton("⏭️ Skip Note", callback_data="note_skip")]])
    await update.message.reply_text("Optional: add a note?", reply_markup=kb)
    return P_EDIT_NOTE
...
async def get_delete_partner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [InlineKeyboardButton(
                  f"[{r.doc_id}] {r['local_amt']:.2f}->{r.get('usd_amt',0):.2f}",
                  callback_data=f"pout_del_sel_{r.doc_id}")
               for r in rows]
    kb = InlineKeyboardMarkup([buttons[i:i + 2] for i in range(0, len(buttons), 2)])
...
def register_payout_handlers(app):
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            ...
            P_NOTE:           [CallbackQueryHandler(get_payout_note, pattern="^note_"),
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            ...
            P_EDIT_NOTE:    [CallbackQueryHandler(get_edit_note, pattern="^(note_skip|edit_note_yes)$"),
