...
async def get_edit_fee(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data['fee_amt'] = data['local_amt'] * fee_pct / 100.0
    await update.message.reply_text(
        f"Fee: {fee_pct:.2f}% → {data['fee_amt']:.2f}. Now enter new USD paid:"
    )
    return P_EDIT_USD
...
async def get_edit_usd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['usd_amt'] = usd
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("📝 Add Note", callback_data="edit_note_yes"),
        InlineKeyboardButton("⏭️ Skip Note", callback_data="note_skip"),
    ]])
    await update.message.reply_text("Optional: add a note?", reply_markup=kb)
    return P_EDIT_NOTE
...
async def confirm_edit_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    data = context.user_data
    net = data['local_amt'] - data['fee_amt']
    fx  = net / data['usd_amt'] if data['usd_amt'] else 0
    summary = (
        f"Local: {data['local_amt']:.2f}\n"
        f"Fee: {data['fee_perc']:.2f}% ({data['fee_amt']:.2f})\n"
        f"USD Paid: {data['usd_amt']:.2f}\n"
        f"FX Rate: {fx:.4f}\n"
        f"Note: {data.get('note','')}"
    )
    kb = InlineKeyboardMarkup([[
...
async def confirm_edit_payout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query.data == 'pout_save_yes':
        ...
        secure_db.update('partner_payouts', updated, [rec0.doc_id])
        await update.callback_query.edit_message_text(
...
def register_payout_handlers(app):
    ...
    add_conv = ConversationHandler(
        ...
        states={
            ...
            P_NOTE:           [
                                CallbackQueryHandler(get_payout_note, pattern="^note_"),
    ...
    edit_conv = ConversationHandler(
        ...
        states={
            ...
            P_EDIT_NOTE:    [
                                CallbackQueryHandler(get_edit_note, pattern="^(note_skip|edit_note_yes)$"),
