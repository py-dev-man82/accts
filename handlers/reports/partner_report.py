...
async def show_partner_report_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        await update.callback_query.edit_message_text(
            "📄 Select a partner to view report:", reply_markup=InlineKeyboardMarkup(grid)
        )
    else:
...
async def show_partner_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_sales = [
        ...
        if s["partner_id"] == pid
        and start_date <= datetime.fromisoformat(s["timestamp"]) <= end_date
    ]
    all_payouts = [
        p
        for p in secure_db.all("partner_payouts")
        if p["partner_id"] == pid
        and start_date <= datetime.fromisoformat(p["timestamp"]) <= end_date
    ]
    all_costs = [
        c
        for c in secure_db.all("partner_inventory")
        if c["partner_id"] == pid
        and start_date <= datetime.strptime(c["date"], "%d%m%Y") <= end_date
    ]
    ...
    # Totals (with safe lookup for unit_cost/cost)
    total_sales = sum(
    ...
    total_costs = sum(
        rec["quantity"] * rec.get("unit_cost", rec.get("cost", 0)) for rec in all_costs
    )
    ...
    if scope in ("full", "sales"):
        ...
        if sales:
            for s in sales:
                ...
                lines.append(
                    f"• {date}: Item {s['item_id']} ×{s['quantity']} @ "
                    f"{fmt_money(s['unit_price'], partner['currency'])} = "
    ...
    if scope in ("full", "costs"):
        ...
        if costs:
            for rec in costs:
                ...
                unit_cost = rec.get("unit_cost", rec.get("cost", 0))
                total_line = rec["quantity"] * unit_cost
                lines.append(
                    f"• {date}: Item {rec['item_id']} ×{rec['quantity']} @ "
                    f"{fmt_money(unit_cost, partner['currency'])} = "
                    f"{fmt_money(total_line, partner['currency'])}"
                )
        else:
...
async def export_pdf_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # PDF generation logic goes here (reuse customer-report implementation)
    buffer = BytesIO()
    ...
    pdf.save()
    buffer.seek(0)
    partner = secure_db.table("partners").get(doc_id=context.user_data["partner_id"])
    start_date = context.user_data["start_date"]
...
def register_partner_report_handlers(app):
    conv = ConversationHandler(
        ...
        states={
            PARTNER_SELECT: [
                CallbackQueryHandler(select_date_range, pattern="^partrep_")
            ],
            DATE_RANGE_SELECT: [
