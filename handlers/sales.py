...
# ────────────────────────────────────────────────────────────────
(
    S_CUST_SELECT,   # Add:  choose customer
    S_STORE_SELECT,  # Add:  choose store
    S_ITEM_QTY,      # Add:  item,qty input
    S_PRICE,         # Add:  unit-price
    S_FEE,           # Add:  handling-fee
    S_NOTE,          # Add:  optional note
    S_CONFIRM,       # Add:  final confirm

    S_EDIT_SELECT,   # Edit: choose customer
    S_EDIT_TIME,     # Edit: choose period
    S_EDIT_PAGE,     # Edit: paginated list
    S_EDIT_FIELD,    # Edit: pick field
    S_EDIT_NEWVAL,   # Edit: enter new value
    S_EDIT_CONFIRM,  # Edit: confirm save

    S_DELETE_SELECT, # Del : choose customer
    S_DELETE_CONFIRM,# Del : confirm del/yes-no

    S_VIEW_CUSTOMER, # View: choose customer
    S_VIEW_TIME,     # View: choose period
    S_VIEW_PAGE      # View: paginated list
) = range(18)
...
async def add_sale(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows.append([InlineKeyboardButton("🔙 Back", callback_data="sales_menu")])  # ← proper row
...
async def get_sale_store(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # show inventory
    Inv = Query()
    inv_rows = secure_db.table("store_inventory").search(Inv.store_id == sid)
...
async def get_sale_item_qty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Inv = Query()
    stock = secure_db.table("store_inventory").get((Inv.store_id == sid) & (Inv.item_id == item_id))
    if not stock or stock["quantity"] < qty:
        available = stock["quantity"] if stock else 0
        await update.message.reply_text(
            f"❌ Not enough stock. Available: {available}. Re-enter item,qty:"
        )
        return S_ITEM_QTY
...
async def get_sale_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Enter handling fee (or Skip):", reply_markup=kb)
    return S_FEE
...
async def get_sale_note(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    summary = (
        f"✅ Confirm Sale\n"
        f"Customer:  {cust['name']} ({cust['currency']})\n"
        f"Store:     {store['name']} ({store['currency']})\n"
        f"Item:      {d['sale_item']}\n"
        f"Quantity:  {d['sale_qty']}\n"
        f"UnitPrice: {d['sale_price']:.2f}\n"
        f"Total:     {total:.2f}\n"
        f"Fee:       {d['sale_fee']:.2f}\n"
        f"Note:      {note or '—'}\n\nConfirm?"
    )
...
async def confirm_sale(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    d = context.user_data
    store_cur = secure_db.table("stores").get(doc_id=d["sale_store"])["currency"]
    ...
    # deduct inventory
    Inv = Query()
    item_rec = secure_db.table("store_inventory").get((Inv.store_id == d["sale_store"]) &
                                                      (Inv.item_id == d["sale_item"]))
    if item_rec:
        secure_db.update("store_inventory",
                         {"quantity": item_rec["quantity"] - d["sale_qty"]},
                         [item_rec.doc_id])
...
async def edit_sale(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        InlineKeyboardButton(f"{c['name']} ({c['currency']})",
...
async def send_edit_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tkey  = context.user_data["edit_time_filter"]
    page  = context.user_data["edit_page"]
    size  = 20

    rows = [r for r in secure_db.all("sales") if r["customer_id"] == cid]
    if tkey in ("3m", "6m"):
        delta = 90 if tkey == "3m" else 180
        cutoff = datetime.utcnow().timestamp() - delta*86400
        rows = [r for r in rows if datetime.fromisoformat(r["timestamp"]).timestamp() >= cutoff]
    ...
    lines = [f"• [{r.doc_id}] Store:{r['store_id']} Item:{r['item_id']} "
             ...
             for r in chunk]
    text = f"✏️ Edit Sales (Page {page}/{total}):\n" + "\n".join(lines)

    nav = []
    if page > 1:             nav.append(InlineKeyboardButton("⬅️ Prev",  callback_data="edit_prev"))
    if page < total:         nav.append(InlineKeyboardButton("➡️ Next",  callback_data="edit_next"))
...
async def get_edit_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "store":
        ...
        await update.callback_query.edit_message_text("Enter new item_id,quantity:")
    elif field == "price":
        await update.callback_query.edit_message_text("Enter new unit price:")
    elif field == "fee":
        await update.callback_query.edit_message_text("Enter new handling fee:")
    elif field == "note":
        await update.callback_query.edit_message_text("Enter new note (or '-' for none):")
    else:
        await update.callback_query.edit_message_text("Invalid field.")
        return await edit_sale(update, context)
...
async def save_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    summary = (f"Edit {field} → {context.user_data['new_value']}\nConfirm?")
    kb = InlineKeyboardMarkup([
...
async def confirm_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "store":
        secure_db.update("sales", {"store_id": int(val)}, [sid])
    elif field == "itemqty":
        item, qty = map(int, val.split(","))
        secure_db.update("sales", {"item_id": item, "quantity": qty}, [sid])
    elif field == "price":
        secure_db.update("sales", {"unit_price": float(val)}, [sid])
    elif field == "fee":
        secure_db.update("sales", {"handling_fee": float(val)}, [sid])
    elif field == "note":
        secure_db.update("sales", {"note": "" if val == "-" else val}, [sid])
...
async def get_delete_customer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [InlineKeyboardButton(f"[{r.doc_id}] {r['item_id']} x{r['quantity']}",
                                    callback_data=f"del_sale_{r.doc_id}")
               for r in rows]
    rows_btn = [[b] for b in buttons]  # one per row
    rows_btn.append([InlineKeyboardButton("🔙 Back", callback_data="sales_menu")])
    await update.callback_query.edit_message_text("Select sale:",
...
async def confirm_delete_sale(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not sale:
        await show_sales_menu(update, context); return ConversationHandler.END
    context.user_data["del_sale"] = sale
    ...
    await update.callback_query.edit_message_text(
        f"Delete sale #{sid}?  This will restore inventory & reverse fee.",
        reply_markup=kb
    )
    return S_DELETE_CONFIRM
...
async def perform_delete_sale(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sale = context.user_data["del_sale"]; sid = context.user_data["del_id"]
    # restore stock
    Inv = Query()
    rec = secure_db.table("store_inventory").get((Inv.store_id == sale["store_id"]) &
                                                 (Inv.item_id == sale["item_id"]))
    if rec:
        secure_db.update("store_inventory",
    ...
    if sale.get("handling_fee",0) > 0:
        secure_db.insert("store_payments", {
            ...
            "note":    f"Reversal fee for del sale #{sid}",
            "timestamp":datetime.utcnow().isoformat()
        })
...
async def send_sales_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tkey = context.user_data["view_time_filter"]
    page = context.user_data["view_page"]; size = 20
    ...
    lines = [f"• [{r.doc_id}] Store:{r['store_id']} Item:{r['item_id']} "
             f"x{r['quantity']} @ {r['unit_price']} = {r['quantity']*r['unit_price']:.2f} {r['currency']}"
             for r in chunk]
    text = f"📄 Sales (Page {page}/{total}):\n" + "\n".join(lines)

    nav = []
    if page>1:          nav.append(InlineKeyboardButton("⬅️ Prev", callback_data="view_prev"))
    if page<total:      nav.append(InlineKeyboardButton("➡️ Next", callback_data="view_next"))
...
delete_conv = ConversationHandler(
    ...
    states={
        ...
        S_DELETE_CONFIRM: [CallbackQueryHandler(confirm_delete_sale, pattern="^del_sale_"),
...
view_conv = ConversationHandler(
    ...
    states={
        ...
        S_VIEW_TIME    : [CallbackQueryHandler(get_view_time,     pattern="^view_time_"),
                          CallbackQueryHandler(view_sales,        pattern="^view_sales$")],
        S_VIEW_PAGE    : [CallbackQueryHandler(handle_pagination, pattern="^view_(prev|next)$"),
