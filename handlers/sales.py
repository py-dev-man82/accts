...
# ───────────────────────────────────────────────────────────────
#  Utility – quick numeric-ID parser
# ───────────────────────────────────────────────────────────────
...
async def get_sale_store(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # show inventory
    q = Query()
    inv = secure_db.table("store_inventory").search(q.store_id == sid)
    inv_txt = ("\n".join([f"• Item {r['item_id']}: {r['quantity']}"
                          for r in inv]) or "No inventory.")
    await update.callback_query.edit_message_text(
...
async def get_sale_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("➖ Skip",
                                                     callback_data="fee_skip")]])
...
async def confirm_sale(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # deduct inventory
    q = Query()
    rec = secure_db.table("store_inventory").get((q.store_id == d["sale_store"]) &
                                                 (q.item_id == d["sale_item"]))
    if rec:
...
# ───────────────────────────────────────────────────────────────
#  EDIT FLOW  (customer → period → page → record)
#  *money formatting added to list*
# ───────────────────────────────────────────────────────────────
@require_unlock
...
async def send_edit_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = []
    for r in chunk:
        sub_total = r["quantity"] * r["unit_price"]
        unit  = fmt_money(r["unit_price"], r["currency"])
        total = fmt_money(sub_total,      r["currency"])
        lines.append(f"{r.doc_id}: Store {r['store_id']}  Item {r['item_id']}  "
                     f"x{r['quantity']} @ {unit} = {total}")
    text = (f"✏️ **Edit Sales**  P{page}/{total}\n\n" + "\n".join(lines) +
            "\n\nReply with record number or use ⬅️➡️")
...
async def select_edit_sale_by_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sid is None:
        await update.message.reply_text("Enter numeric record ID.")
        return S_EDIT_PAGE
...
async def get_edit_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "store":
        ...
        await update.callback_query.edit_message_text("New item_id,quantity (e.g. 5,20):")
    elif field == "price":
        await update.callback_query.edit_message_text("New unit price:")
    elif field == "fee":
        await update.callback_query.edit_message_text("New handling fee:")
    elif field == "note":
...
async def save_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Change **{field}** → `{context.user_data['new_value']}`  ?",
        reply_markup=kb
    )
...
# ───────────────────────────────────────────────────────────────
#  DELETE FLOW  – list strings use fmt_money now
# ───────────────────────────────────────────────────────────────
@require_unlock
...
async def get_delete_customer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = []
    for r in rows:
        total = fmt_money(r["quantity"]*r["unit_price"], r["currency"])
        lines.append(f"{r.doc_id}: Item {r['item_id']} ×{r['quantity']} = {total}")
    await update.callback_query.edit_message_text(
        "Reply with record number to delete:\n\n" + "\n".join(lines),
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",
                                                                 callback_data="sales_menu")]])
    )
...
async def perform_delete_sale(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # restore stock
    q = Query()
    rec = secure_db.table("store_inventory").get((q.store_id == sale["store_id"]) &
                                                 (q.item_id == sale["item_id"]))
    if rec:
        secure_db.update("store_inventory",
    ...
    if sale.get("handling_fee",0) > 0:
        secure_db.insert("store_payments", {
            ...
            "note":     f"Fee reversal for deleted sale #{sid}",
            "timestamp":datetime.utcnow().isoformat(),
        })
...
# ───────────────────────────────────────────────────────────────
#  VIEW FLOW  – money formatting on list lines
# ───────────────────────────────────────────────────────────────
@require_unlock
...
async def send_sales_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in chunk:
        unit  = fmt_money(r["unit_price"], r["currency"])
        total_amt = fmt_money(r["quantity"]*r["unit_price"], r["currency"])
        lines.append(f"{r.doc_id}: Store {r['store_id']}  Item {r['item_id']}  "
                     f"x{r['quantity']} @ {unit} = {total_amt}")
    text = f"📄 **Sales**  P{page}/{total}\n\n" + "\n".join(lines)
